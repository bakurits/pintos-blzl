...
void thread_init(void) {
  ...
  set_initial_thread_values(initial_thread);
  load_avg = fix_int(0);
}
...
void calculate_new_recent_cpu(void) { thread_foreach(recent_cpu_update, NULL); }
...
// This method would be called for each thread to calculate new cpu - aux not
// used
void recent_cpu_update(struct thread *t, void *aux UNUSED) {
  ...
  t->recent_cpu = fix_add(
      fix_mul(fix_div(fix_mul(load_avg, fix_int(2)),
...
void mlfq_priority_update(struct thread *t, void *aux UNUSED) {
  ...
  t->priority =
      PRI_MAX - fix_round(fix_unscale(t->recent_cpu, 4)) - (t->nice * 2);
}

// update current value of load_avg
void calculate_new_load_avg(void) {
  // load_avg = (59/60) × load_avg + (1/60) × ready_threads
  load_avg = fix_div(fix_add(fix_mul(fix_int(59), load_avg),
                             fix_int(get_ready_threads_count())),
                     fix_int(60));
}

// Returns "number of threads that are either running or ready to run at time of
// update"
int get_ready_threads_count(void) {
...
bool thread_awake_time_cmp(const struct list_elem *a, const struct list_elem *b,
                           void *aux UNUSED) {
