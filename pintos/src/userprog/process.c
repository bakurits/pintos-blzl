...
static size_t get_args_cnt(const char *st) {
  int i = 0;
  size_t ans = 0;
...
static void *load_args(void *st_pointer, struct args_data_t *data) {
  char *st_ptr = st_pointer;
  size_t argc = data->argc;
  char **argv = data->argv;
  int i;
  for (i = (int)argc - 1; i >= 0; i--) {
    st_ptr -= strlen(argv[i]) + 1;
    strlcpy(st_ptr, argv[i], strlen(argv[i]) + 1);
  }

  char *argument_values_offset = st_ptr;
  while ((size_t)st_ptr % 4 != 0) {
    st_ptr--;
  }
  st_ptr -= sizeof(char *);
  *((char **)st_ptr) = NULL;

  st_ptr -= argc * sizeof(char *);
...
tid_t process_execute(const char *args) {
  ...
  size_t argc = get_args_cnt(fn_copy);
  char *argv[argc];
  int ind = 0;
  char *saveptr1;
  /* get the first token */
  argv[ind++] = strtok_r(fn_copy, " ", &saveptr1);

  /* walk through other tokens */
  while (argv[ind - 1] != NULL) {
    argv[ind++] = strtok_r(NULL, " ", &saveptr1);
  }
...
/* A thread function that loads a user process and starts it
   running. */
static void start_process(void *argv) {
  struct args_data_t *args_data = (struct args_data_t *)argv;
...
/* Loads a segment starting at offset OFS in FILE at address
   ...
   memory are initialized, as follows:
                                ...
                                - READ_BYTES bytes at UPAGE must be read from
   FILE starting at offset OFS.

                                - ZERO_BYTES bytes at UPAGE + READ_BYTES must be
   zeroed.
